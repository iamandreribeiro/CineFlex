...
let idsSelecionados = [];
let namesSelecionados = [];

export default function SeatsScreen(props) {
    const URL =
    `https://mock-api.driven.com.br/api/v3/cineflex/showtimes/` +
    props.idSessao +
    `/seats`;
  ...
  useEffect(() => {
    ...
    promise.then((s) => {
      ...
      console.log(s.data.seats);
    });
  }, [URL]);

  function selecionaAssento(a) {
    if (a.isAvailable && !props.selecionados.includes(a.name)) {
      idsSelecionados = [...props.selecionados, a.id];
      namesSelecionados = [...props.assentos, a.name];
    } else alert("Esse assento não está disponível");
    props.setSelecionados(idsSelecionados);
    props.setAssentos(namesSelecionados);
  ...
  function fazReserva(reserva) {
    const URL =
      "https://mock-api.driven.com.br/api/v3/cineflex/seats/book-many";
        ...
        {assentos.map((a, i) => {
          return (
            <StyledSeatButton
              ...
              color={a.isAvailable}
              bgcolor={props.selecionados.includes(a.id)}
            >
              {a.name}
            </StyledSeatButton>
          );
...
const StyledSeatButton = styled.button`
  background-color: ${(props) =>
    props.color
      ? (props) => (props.bgcolor ? "#1AAE9E" : "#C3CFD9")
      : "#FBE192"};
  border: 1px solid
    ${(props) =>
      props.color
        ? (props) => (props.bgcolor ? "#0E7D71" : "#808F9D")
        : "#F7C52B"};
  width: 30px;
  height: 30px;
  border-radius: 50%;
